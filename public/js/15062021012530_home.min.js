...
break;default:error.insertAfter(element);}},submitHandler:function(form){this.settings.beforeAjax(this);this.settings.ajax(this);},},};objValidation=$.extend(true,settings,options);obj.validate(objValidation.validation);$.extend(obj.data("validator").settings,objValidation.defaults);});};})(jQuery);function getUtmParams(){utmParams=[];ref=UtmForm._utmParamsMap;for(param in ref){if(UtmForm.utmCookie.readCookie(param)!=null)
...
document.getElementsByTagName("body")[0].appendChild(js);},};var locationLoader=function(){};locationLoader.prototype=Object.create(new customLoader());locationLoader.prototype._onEvent=function($this){if($this.isLoaded==0){objLocation=$.cookie("user-location");if(objLocation!==undefined){fillFormFields();}}
$(document).on("touchmove mousewheel mousemove mousescroll",function(){if($this.isLoaded==0)$this._loadScript($this);});window.onscroll=function(e){if($this.isLoaded==0)$this._loadScript($this);};};locationLoader.prototype._loadScript=function($this){$this.isLoaded=1;objLocation=$.cookie("user-location");if(objLocation!==undefined)return $this._setLocationFields();$.ajax({type:"get",url:base_url+"location/ajax_set_location",data:{city_id:objLocation?objLocation.city.id:0},dataType:"json",async:true,cache:false,headers:{"cache-control":"no-cache"},}).done(function(ajaxResult,textStatus,jqXHR){if(jqXHR.readyState==4&&jqXHR.status==200){if(typeof ajaxResult==="undefined"||typeof ajaxResult.result==="undefined")
return;if(ajaxResult.success==true){objLocation=ajaxResult.result.location;$.cookie("user-location",objLocation,{expires:365,path:"/"});}
$this._setLocationFields();}});};locationLoader.prototype._setLocationFields=function(){$.event.trigger({type:"location:found",});objLocation=$.cookie("user-location");objFormFields={};if(objLoggedUser.logged&&objLoggedUser.country_id){objFormFields=$.extend(true,objFormFields,{country_code:objLoggedUser.country_code!=""?objLoggedUser.country_code:objLocation.country.code,country_id:objLoggedUser.country_id,city_id:objLoggedUser.city_id,isd:objLoggedUser.isd!=""?objLoggedUser.isd:objLocation.isd,});}else{objFormFields=$.extend(true,objFormFields,{country_code:objLocation.country.code,country_id:objLocation.country.id,city_id:objLocation.city.id,isd:objLocation.country.isd,});}
objCookieFormFields=$.cookie("form-fields");objCookieFormFields===undefined?$.cookie("form-fields",objFormFields,{expires:365,path:"/"}):$.cookie("form-fields",$.extend(true,objCookieFormFields,objFormFields));fillFormFields();};function fillFormFields(){objLocation=$.extend(true,{},$.cookie("user-location"));objFormFields=$.extend(true,{country_id:0,city_id:0,currency_id:0,course_id:0,isd:0,},$.cookie("form-fields"));if(Object.keys(objLocation).length!==0&&objLocation.region.slug=="europe"){$('input[name="agree_gdpr"][type="checkbox"]').each(function(){$(this).closest(".col-md-12").removeClass("hide");});var cookieBotLoader=cookieBotLoader||new customLoader();cookieBotLoader.init({id:"Cookiebot",src:"//consent.cookiebot.com/uc.js",attributes:{"data-cbid":"034ff55d-3ac9-4a93-83f2-f019e40940ae","data-blockingmode":"auto",},_onEvent:function($this){if($this.isLoaded==0)$this._loadScript($this);},});}
$.each(objFormFields,function(field,value){switch(field){case"country_id":if(typeof countries!="undefined"&&$('select[name="country_id"][data-dropdown-type="trending"]').length)
$('select[name="country_id"][data-dropdown-type="trending"]').dropdown({method:"trending",values:countries,sorting:false,selected:value,prepend:'<option value="">Select Country</option>',}).selectpicker({dropupAuto:"true",size:"8",});$(".global-form select.customize-select").floatingLabel();break;case"isd":if(typeof countries!="undefined"&&$('select[name="isd"][data-dropdown-type="trending"]').length)
$('select[name="isd"][data-dropdown-type="trending"]').dropdown({method:"isd-trending",values:countries,columns:["value","name"],sorting:false,selected:objFormFields.country_id,prepend:'<option value="">Select ISD</option>',}).selectpicker({dropupAuto:"true",size:"8",});break;case"currency_id":if(typeof currencies!="undefined"&&$('select[name="currency_id"]').length)
$('select[name="currency_id"]').dropdown({values:currencies,columns:["id","name"],selected:objFormFields.country_id,prepend:'<option value="">Select Currency</option>',});break;case"city_id":if($('select[name="city_id"]').length)
$('select[name="city_id"]').dropdown({method:"city",parent:objFormFields.country_id,sorting:false,selected:value,prepend:'<option value="">Select City</option>',}).selectpicker({dropupAuto:"true",size:"8",noneSelectedText:"Select City",}).selectpicker("refresh");break;case"course_id":if(typeof courses!="undefined"&&$('select[name="course_id"][data-dropdown-type="trending"]').length)
$('select[name="course_id"][data-dropdown-type="trending"]').dropdown({method:"trending",values:courses,sorting:false,selected:value,prepend:'<option value="">Select Course</option>',}).selectpicker({dropupAuto:"true",size:"8",});$(".global-form select.customize-select").floatingLabel();break;default:$(document).find('input[name="'+
field+'"], textarea[name="'+
field+'"], select[name="'+
field+'"]').val(value);break;}});}
(function($){$.fn.populate=function(){if(!this.length)return this;return this.each(function(){var $obj=$(this);$obj.on("change",function(){if(!$obj.val())return;var $parent=$obj.attr("data-parent")?$(document).find($obj.attr("data-parent")):$obj.closest("form");var $city=$obj.attr("data-target")?$parent.find($obj.attr("data-target")):$parent.find('select[name="city_id"]');var $currency=$obj.attr("data-target")?$parent.find($obj.attr("data-target")):$parent.find('select[name="currency_id"]');var $isd=$obj.attr("data-target")?$parent.find($obj.attr("data-target")):$parent.find('select[name="isd"]');if($city.length){$city.html("").dropdown({method:"city",parent:parseInt($obj.val()),sorting:false,prepend:'<option value="">Select City</option>',}).selectpicker("refresh");$(".global-form select.customize-select").floatingLabel();}
$currency.length?$currency.dropdown({method:"currency",country_id:$obj.val()}):false;if($isd.length){$isd.dropdown({method:"isd-trending",values:countries,columns:["value","name"],sorting:false,selected:parseInt($obj.val()),prepend:'<option value="">Select ISD</option>',}).selectpicker("refresh");}});});};})(jQuery);(function($){$.fn.reset=function(){var $obj=$(this);$obj.find("input, textarea, select").not(":input[type=hidden], :input[type=button], :input[type=submit], :input[type=reset], select[name=isd]").val("").removeAttr("selected").removeAttr("checked").removeClass("used");$obj.find(".selectpicker").selectpicker("refresh");$obj.find(".error").remove();};})(jQuery);(function($){$.fn.setFormValuesInCookie=function(){if(!this.length)return this;$obj=$(this);arrFields=["name","first_name","last_name","email","isd","phone","organization","company","designation","job_title","address1","country_id","city_id","course_id",];objFields={};$.each(arrFields,function(key,field){fieldValue=$obj.find("input[name='"+field+"'], select[name='"+field+"']").val();if(typeof fieldValue!="undefined"&&fieldValue.length)
objFields[field]=fieldValue;});objFormFields=$.cookie("form-fields");objFormFields===undefined?$.cookie("form-fields",objFields,{expires:365,path:"/"}):$.cookie("form-fields",$.extend(true,objFormFields,objFields));fillFormFields();};})(jQuery);function notify(status,message){if(status=="success")
$.growl.notice({title:"Success",message:message,});else
$.growl.error({title:"Error",message:message,});}
(function($){$.fn.sortDropdown=function(){var $obj=$(this);var $value=$obj.val();$obj.append($obj.find("option").remove().sort(function(fElement,nElement){(fElementText=$(fElement).text()),(nElementText=$(nElement).text());return fElementText>nElementText?1:fElementText<nElementText?-1:0;}));$obj.val($value);};})(jQuery);(function($){$.fn.goTop=function(options){if(!this.length)return;var settings=$.extend(true,{scrollSpeed:"slow",animationSpeed:400,fadeInStartPosition:600,},options);return this.each(function(){var $obj=$(this);$(window).scroll(function(){if($(this).scrollTop()>=settings.fadeInStartPosition){if(typeof $obj.attr("data-fade")=="undefined"||$obj.attr("data-fade")=="false")
$obj.attr("data-fade",true).fadeIn(settings.animationSpeed);}else{if(typeof $obj.attr("data-fade")=="undefined"||$obj.attr("data-fade")=="true")
$obj.attr("data-fade","false").fadeOut(settings.animationSpeed);}});$obj.on("click",function(e){e.preventDefault();$obj.attr("data-fade","false");$("body, html").animate({scrollTop:0,},settings.scrollSpeed);document.activeElement.blur();});});};})(jQuery);(function($){$.fn.scrollTo=function(){if(!this.length)return this;return this.each(function(){var $obj=$(this);$obj.on("click",function(e){e.preventDefault();if($($obj.attr("data-target")).length)
$($obj.attr("data-target")).scrollToElem();return false;});});};})(jQuery);(function($){$.fn.scrollToElem=function(){if(!this.length)return this;var $obj=$(this);$("html, body").stop().animate({scrollTop:$obj.offset().top,},"slow");};})(jQuery);(function($){var $searchCourses=[];$.each(courses,function(i,e){var matchingItems=$.grep($searchCourses,function(item){return item.id===e.id;});if(matchingItems.length===0){$searchCourses.push(e);}});$.fn.searchCourses=function(){if(!this.length)return;$(".courses-hover, .menu-dropdown").on("mouseover",function(e){var container=$(".autocomplete-items");if(!container.is(":hidden")&&!container.is(e.target)&&container.has(e.target).length===0){container.hide();}
$('input[name="q"]').blur();});$(".search-btn").on("click",function(){$(".search-field").removeClass("d-none");$(".close-search").removeClass('hide');});$(".close-search").on("click",function(){$(".search-field").addClass("d-none");});$('.header-search-reset, .m-header-search-reset').click(function(){var $obj=$(this);$obj.parent().find(".autocomplete-items").remove();$(".course-search-form input[name='q']").val('');$obj.addClass('hide');});$(document).mouseup(function(e){var container=$(".autocomplete-items");if(!container.is(e.target)&&container.has(e.target).length===0){container.hide();}
if(e.target.name!="q"&&e.target.className!="f-icons f-search-icon"&&e.target.className!="header-search-icon")
{$(".course-search-form input[name='q']").val('');$('.header-search-reset').addClass('hide');}});return this.each(function(){var $obj=$(this);$obj.on("keyup click",function(){var $val=$obj.val().toUpperCase();$obj.parent().find(".autocomplete-items").remove();$('.header-search-reset, .m-header-search-reset').removeClass('hide');var $lis=[];if($val!==""){$.each($searchCourses,function(i,course){$position=course.name.toUpperCase().indexOf($val);var findPattern=new RegExp("("+$val+")","gi");if($position>-1&&$lis.length<8){$lis.push('<li><a href="'+
base_url+
course.url+'">'+
course.name.replace(findPattern,"<span style='font-weight:600;'>$1</span>")+"</a></li>");}});}else{$.each(courses,function(i,course){if(course.trending==1&&$lis.length<8){$lis.push('<li><a href="'+
base_url+
course.url+'">'+
course.name+"</a></li>");}});}
if($lis.length){$obj.after('<div class="autocomplete-items"><ul class="auto-complete-courses-list">'+
$lis.join("")+"</ul></div>");}});});};})(jQuery);(function($){$.fn.viewMore=function(options){if(!this.length)return;var settings=$.extend(true,{parentClass:".custom-cards",childClass:".custom-thumb",length:3,},options);return this.each(function(){var $obj=$(this);viewMoreObject=$obj.closest(settings.parentClass).find(settings.childClass);viewMoreObject.slice(0,settings.length).slideDown();if(viewMoreObject.length<settings.length)$obj.addClass("hide");$obj.on("click",function(){$obj.closest(settings.parentClass).find(settings.childClass).slideDown();$obj.fadeOut("slow");});});};})(jQuery);(function($){$.fn.button=function(action){if(!this.length)return;return this.each(function(){var $obj=$(this);if(action==="loading"&&$obj.data("loading-text")){$obj.data("original-text",$obj.html()).html($obj.data("loading-text")).prop("disabled",true);}else if(action==="reset"&&$obj.data("original-text")){$obj.html($obj.data("original-text")).prop("disabled",false);}});};})(jQuery);function AddReadMore(){var carLmt=310;var readMoreTxt=" Read More";var readLessTxt=" Read Less";$(".addReadMore").each(function(){if($(this).find(".firstSec").length)return;var allstr=$(this).text();if(allstr.length>carLmt){var firstSet=allstr.substring(0,carLmt);var secdHalf=allstr.substring(carLmt,allstr.length);var strtoadd="<p>"+
firstSet+"<span class='SecSec'>"+
secdHalf+"</span><span class='readMore'  title='Click to Show More'>"+
readMoreTxt+"</span><span class='readLess' title='Click to Show Less'>"+
readLessTxt+"</span>";$(this).html(strtoadd);}});$(document).on("click",".readMore,.readLess",function(){$(this).closest(".addReadMore").toggleClass("showlesscontent showmorecontent");});}
(function($){$.fn.customSwiper=function(options){if(!this.length)return;var settings=$.extend({},{slidesPerView:4,spaceBetween:16,breakpointsInverse:true,pagination:{el:".swiper-pagination",clickable:false,},navigation:{nextEl:".swiper-button-next",prevEl:".swiper-button-prev",},watchOverflow:true,breakpoints:{1024:{slidesPerView:4,spaceBetween:16,},768:{slidesPerView:2,spaceBetween:12,},640:{slidesPerView:3,spaceBetween:12,},320:{slidesPerView:1.2,spaceBetween:12,},},},options);if(this.length>1){return this.each(function(index,element){$this=$(this);$this.addClass("instance-"+index);if(typeof settings.pagination.el!="undefined"){paginationEl="swiper-pagination-"+index;if($this.parent().find(settings.pagination.el).length){$this.parent().find(settings.pagination.el).addClass(paginationEl);}
if($this.find(settings.pagination.el).length){$this.find(settings.pagination.el).addClass(paginationEl);}
settings.pagination.el=$this.parent().find("."+paginationEl)[0]||$this.find("."+paginationEl)[0];}
if(typeof settings.navigation.nextEl!="undefined"){navigationNextEl="swiper-button-next-"+index;if($this.parent().find(settings.navigation.nextEl).length){$this.parent().find(settings.navigation.nextEl).addClass(navigationNextEl);}
if($this.find(settings.navigation.nextEl).length){$this.find(settings.navigation.nextEl).addClass(navigationNextEl);}
settings.navigation.nextEl=$this.parent().find("."+navigationNextEl)||$this.find("."+navigationNextEl);}
if(typeof settings.navigation.prevEl!="undefined"){navigationPrevEl="swiper-button-prev-"+index;if($this.parent().find(settings.navigation.nextEl).length){$this.parent().find(settings.navigation.nextEl).addClass(navigationPrevEl);}
if($this.find(settings.navigation.nextEl).length){$this.find(settings.navigation.nextEl).addClass(navigationPrevEl);}
settings.navigation.prevEl=$this.parent().find("."+navigationPrevEl)||$this.find("."+navigationPrevEl);}
return new Swiper(".instance-"+index,settings);});}else{if(typeof settings.pagination.el!="undefined"){settings.pagination.el=$(this).parent().find(settings.pagination.el)[0]||$(this).find(settings.pagination.el)[0];}
if(typeof settings.navigation.nextEl!="undefined"){settings.navigation.nextEl=$(this).parent().find(settings.navigation.nextEl)||$(this).find(settings.navigation.nextEl);}
if(typeof settings.navigation.prevEl!="undefined"){settings.navigation.prevEl=$(this).parent().find(settings.navigation.prevEl)||$(this).find(settings.navigation.prevEl);}
return new Swiper(this,settings);}};})(jQuery);(function($){$.fn.acceptCookie=function(options){if(!this.length)return;var settings=$.extend({},{name:"accept-cookie",},options);return this.each(function(){var $obj=$(this);if(typeof $.cookie(settings.name)==="undefined"){$(window).scroll(function(){$obj.is(":hidden")&&typeof $.cookie(settings.name)==="undefined"?$obj.css("display","block"):false;});$obj.find(".btn-policy").on("click",function(){$.cookie(settings.name,1,{expires:365,path:"/"});$obj.css("display","none");$.cookie(settings.name,1,{expires:365,path:"/"});});}});};})(jQuery);var lazyLoader=function(){};lazyLoader.prototype={elementSelector:".lazy",parentSelector:"",init:function(params){if(params){for(var param in params)this[param]=params[param];}
...
$(document).on("touchmove mousewheel mousemove mousescroll",function(){$this._lazyLoad($this);});window.onscroll=function(e){$this._lazyLoad($this);};},_lazyLoad:function($this){if(!$($this.elementSelector).length)return;var hT=$($this.elementSelector).first().offset().top,hH=$($this.elementSelector).first().outerHeight(),wH=$(window).height(),wS=$(window).scrollTop();if(wS>hT+hH-wH){$($this.elementSelector).each(function(){$obj=$(this);if(!$obj.attr("is-loaded"))
$obj.attr("src",$obj.attr("data-src")).attr("is-loaded",true);});}},};var lazyLoadImages=new lazyLoader();lazyLoadImages.init();$(function(){AddReadMore();$('.course-search-form input[name="q"], .course-search-mobile input[name="q"]').searchCourses();$('[data-toggle="go-top"]').goTop();$('[data-toggle="scroll-to"]').scrollTo();$('[data-toggle="populate"]').populate();$(".fixed-cookie").acceptCookie();});var backgroundImagesLoader=new customLoader();backgroundImagesLoader.init({_onEvent:function($this){if($(window).scrollTop()>100){if($this.isLoaded==0)$this._loadScript($this);}else{$(document).on("touchmove mousewheel mousemove mousescroll",function(){if($this.isLoaded==0)$this._loadScript($this);});window.onscroll=function(e){if($this.isLoaded==0)$this._loadScript($this);};}},_loadForcely:function(){this._loadScript(this);},_loadScript:function($this){$this.isLoaded=1;var css={"background-repeat":"no-repeat","height":"82px","width":"82px","display":"inline-block","transform":"translateY(-50%) scale(0.7)","position":"absolute","top":"50%"};backgroundImages={".accrediation":"https://d2o2utebsixu4k.cloudfront.net/assets/icons/acd-sprite.png",".accrediation-sprite":"https://d2o2utebsixu4k.cloudfront.net/assets/icons/acd-sprite.png",".how-why":"https://d2o2utebsixu4k.cloudfront.net/assets/icons/how-sprite.png",".icons-course":"https://d2o2utebsixu4k.cloudfront.net/assets/icons/course-sprite-32.png",".color-clients-icons":"https://d2o2utebsixu4k.cloudfront.net/assets/icons/color-clients-icons.png",".common-icons":"https://d2o2utebsixu4k.cloudfront.net/assets/icons/icon-sprite.png"};$.each(backgroundImages,function(className,url){$(document).find(className).css("background-image",'url("'+url+'")');if(className=='.icons-course')
$(document).find(className).css(css);else
$(document).find(className).css("background-repeat","no-repeat");});},});var modalsLoader=new customLoader();modalsLoader.init({_onEvent:function($this){$('[data-toggle="modal"], .nav-togglebar').bind("mouseenter click tap",function(){if($this.isLoaded==0)$this._loadScript($this);});$(document).on("touchmove mousewheel mousemove mousescroll",function(){if($this.isLoaded==0)$this._loadScript($this);});window.onscroll=function(e){if($this.isLoaded==0)$this._loadScript($this);};},_loadScript:function($this){$this.isLoaded=1;var objPostParams=$.extend({},{course_id:0},JSON.parse(objModalParams));$.ajax({type:"get",url:base_url+"modals/ajax_get_3_0_modals",data:objPostParams,async:true,cache:false,dataType:"json",headers:{"cache-control":"no-cache"},}).done(function(ajaxResult,textStatus,jqXHR){if(jqXHR.readyState==4&&jqXHR.status==200){loadTopBanners();$("body").append(ajaxResult.result.modals);$("header").after(ajaxResult.result.mega_menu);$(".courses-hover, .menu-dropdown").on({mouseover:function(){$(".menu-dropdown").css({opacity:"1",visibility:"visible"});},mouseout:function(){$(".menu-dropdown").css({opacity:"0",visibility:"hidden"});},});$(".menu-dropdown").on("mouseleave",function(){$submenu=$(".submenu-main");$submenu.find("li.active .sub-menu").addClass("hidden");$submenu.find("li.active .view-trending").addClass("hidden");$submenu.find("li.active").removeClass("active");$submenu.find("li:first-child").addClass("active");$submenu.find("li:first-child").find(".sub-menu").removeClass("hidden");$submenu.find("li:first-child").find(".view-trending").removeClass("hidden");});$(".submenu-main > li").hover(function(){$this=$(this);if($this.hasClass("active"))return;$submenu=$(".submenu-main");$submenu.find("li.active .sub-menu").addClass("hidden");$submenu.find("li.active .view-trending").addClass("hidden");$submenu.find("li.active").removeClass("active");$this.addClass("active");$this.find(".sub-menu").removeClass("hidden");$this.find(".view-trending").removeClass("hidden");});form3_0Validations();fillFormFields();$('[data-toggle="populate"]').populate();backgroundImagesLoader._loadForcely();selectHighlighter();}});},});function addZopimLead(options){leadData=$.extend(true,getUtmParams(),options);leadData["page"]=current_url;objFormFields=$.extend(true,{course_id:0,},$.cookie("form-fields"));leadData["course_id"]=objFormFields.course_id;leadData=Object.entries(leadData).map(function(item){return{name:item[0],value:item[1]};});$.ajax({type:"post",url:base_url+"forms/ajax_add_zopim_lead",data:leadData,dataType:"json",async:true,cache:false,}).done(function(ajaxResponse,textStatus,jqXHR){if(jqXHR.readyState==4&&jqXHR.status==200){if(typeof ajaxResponse==="undefined")return;}});}
var zopimLoader=new customLoader();zopimLoader.init({id:"id-zopim",_loadScript:function($this){$this.isLoaded=1;if(environment!="production")return;if(zopimLoaded||document.getElementById($this.id))return;window.$zopim||(function(e,t){var n=($zopim=function(e){n._.push(e);}),i=(n.s=e.createElement(t)),r=e.getElementsByTagName(t)[0];(n.set=function(e){n.set._.push(e);}),(n._=[]),(n.set._=[]),(i.async=!0),i.setAttribute("charset","utf-8"),(i.src="//v2.zopim.com/?1nYTxEgVmvd5mYJ4Oo8tLPC5gZzJ3OsZ"),(n.t=+new Date()),(i.type="text/javascript"),r.parentNode.insertBefore(i,r);})(document,"script");$this._postLead($this);},_postLead:function($this){if($this.isLoaded){$zopim(function(){$zopim.livechat.setOnChatStart(function(){urmParams=getUtmParams();$.each(urmParams,function(key,value){if(value===""||value===null){delete urmParams[key];}});$zopim.livechat.appendNotes(JSON.stringify(urmParams));addZopimLead({first_name:$zopim.livechat.getName(),email:$zopim.livechat.getEmail(),phone:$zopim.livechat.getPhone(),});});});}},});(function($){$.fn.loader=function(){if(!this.length)return this;return this.each(function(){var $obj=$(this);$obj.on("click tap",function(e){if($obj.find(".loadingspinner").length)return;spinnerLoaderText=$obj.attr("data-loader-text");spinnerLoaderText?$obj.html("<i class='loadingspinner'></i>  "+spinnerLoaderText):$obj.prepend("<i class='loadingspinner'></i>");});});}})(jQuery);$('.loader').loader();(function($){$.fn.selectHighlighter=function(){if(!this.length)return this;return this.each(function(){var $obj=$(this);var myvalue=$(this).val();if(myvalue!==""){$obj.parent().addClass("valid");}});}})(jQuery);function selectHighlighter(){$('.form-control select[name !="isd"]').each(function(){var myvalue=$(this).val();if(myvalue!==""){$(this).parent().addClass("valid");}});}
$(document).on('change','.form-control select[name !="isd"]',function(){if($(this).attr('data-id')=='remove-show-error')
return false;var myvalue=$(this).val();if(myvalue===""){$(this).parent().removeClass('valid').addClass("show-error");}else{$(this).parent().removeClass('show-error').addClass("valid");}});var urlParams=new URLSearchParams(window.location.search);if(urlParams.has('admitad_uid'))
{if(urlParams.get('admitad_uid')!='')
$.cookie("admitad_uid",urlParams.get('admitad_uid'),{expires:30,path:"/"})}
(function($){$.fn.floatingLabel=function(){if(!this.length)return this;return this.each(function(){var $obj=$(this);floatingDefinition($obj);$obj.on("change",function(){floatingDefinition($obj);});});function floatingDefinition($obj)
{if(!$obj.val())
{$obj.closest('.select-options').find('.select-floating-label').removeClass("value-selected");}
else
{$obj.closest('.select-options').find('.select-floating-label').addClass("value-selected");}}};})(jQuery);$(window).scroll(function(){if($(window).scrollTop()>500){$('.fixed-sticky-bar').css('bottom','0px');$('.footer-sticky-bar').css('bottom','0px');$('.mobile-sticky-footer').css('bottom','0px');}});$('.fixed-cookie a').on("click",function(){$(".fixed-cookie").hide();});$(".mobile-sidebar .mobile-sidebar-body").on("click",function(e){$(".mobile-sidebar .mobile-sidebar-body").css('height','80%');});$(window).bind("load resize",function(e){e.preventDefault();if(window.innerWidth<=1240){$('.navbar-toggler').click(function(e){e.preventDefault();if($('.small-screen-header navbar-collapse').hasClass("show")){$('.small-screen-header navbar-collapse').removeClass("show");$('html, body').css('overflowY','auto');$('.body-overlay').css('opacity','0').css('z-index','-1');}else{$('.small-screen-header navbar-collapse').addClass("show");$('html, body').css('overflow','hidden');$('.body-overlay').css('opacity','1').css('z-index','9');}});$(".nav-close").on("click",function(e){if($('#navbar-collapse').hasClass("show")){$('#navbar-collapse').removeClass("show");$('.body-overlay').css('opacity','0').css('z-index','-1');$('html, body').css('overflowY','auto');}});}});jQuery.fn.highlight=function(pat){function innerHighlight(node,pat){var skip=0;if(node.nodeType==3){var pos=node.data.toUpperCase().indexOf(pat);pos-=(node.data.substr(0,pos).toUpperCase().length-node.data.substr(0,pos).length);if(pos>=0){var spannode=document.createElement('span');spannode.className='highlight';var middlebit=node.splitText(pos);var endbit=middlebit.splitText(pat.length);var middleclone=middlebit.cloneNode(true);spannode.appendChild(middleclone);middlebit.parentNode.replaceChild(spannode,middlebit);skip=1;}}
...
if(form.response.success==true){$(".corporate-page-assessment-popup-model .corporate-page-assessment-popup-success-form").removeClass("d-none");$(".corporate-page-assessment-popup-model .corporate-page-popup-form").addClass("d-none");form.settings.trackPixel(form);$(form.currentForm).setFormValuesInCookie();form.settings.reset(form);}
...
if(form.response.success==true){$(".corporate-page-demo-popup-model .corporate-page-demo-popup-success-form").removeClass("d-none");$(".corporate-page-demo-popup-model .corporate-page-popup-form").addClass("d-none");form.settings.trackPixel(form);$(form.currentForm).setFormValuesInCookie();form.settings.reset(form);}
...
key+'"], select[name="'+
key+'"]').after(value);});}}
if(form.response.success==true){$(form.currentForm).closest('.modal').removeClass('try-for-free-modal').addClass('corporate-page-popup-model');$(form.currentForm).openSuccessTemplate();form.settings.trackPixel(form);$(form.currentForm).setFormValuesInCookie();form.settings.reset(form);}
...
(function($){$.fn.openSuccessTemplate=function(){if(!this.length)
return;return this.each(function(){var $this=$(this);var $modal=$(this).closest('.modal');switch($modal.attr('data-custom-modal-class')){case"close-success-modal":$modal.find(".corporate-page-popup-success-form").removeClass("d-none");$modal.find(".corporate-page-popup-form").addClass("d-none");break;default:$modal.animate({scrollTop:0});$modal.find("[data-class='before_success']").before($('#form-success-template').html());$modal.find("[data-class='before_success']").addClass('d-none');$modal.find('.on_success').removeClass('d-none');width_of_referral_modal=$this.find('.bg-white').width();$this.closest(".modal-dialog").css({'max-width':'887px'});break;}
$modal.closeSuccessTemplate();});};})(jQuery);(function($){$.fn.closeSuccessTemplate=function(){if(!this.length)
return;return this.each(function(){var $this=$(this);switch($this.attr('data-custom-modal-class')){case"close-success-modal":$this.on('hidden.bs.modal',function(){$this.find(".corporate-page-popup-success-form").addClass("d-none");$this.find(".corporate-page-popup-form").removeClass("d-none");});break;default:$this.on('hidden.bs.modal',function(){$this.find(".modal-dialog").removeAttr("style");$this.find('.on_success, .on_success_message').remove();$this.find("[data-class='before_success']").removeClass('.d-none');}).on('shown.bs.modal',function(){$this.find('.on_success, .on_success_message').remove();$this.find("[data-class='before_success']").removeClass('d-none');});break;}});};})(jQuery);(function($){$.fn.trendingCourse=function(){if(!this.length)return;return this.each(function(){var $obj=$(this);$obj.on("click",function(){if(typeof $obj.data("value")!="number")return;$.ajax({type:"get",url:base_url+"courses/ajax_get_home_courses_by_name",data:{search:$obj.data("value"),},dataType:"json",async:true,cache:false,}).done(function(ajaxResponse,textStatus,jqXHR){if(jqXHR.readyState==4&&jqXHR.status==200){if(ajaxResponse.result.response==true){if(typeof ajaxResponse.result.view!="undefined"){$obj.closest(".trending-courses").find(".trending-courses-list").html(ajaxResponse.result.view);trendSlider.update();$obj.val()!=0?$(".trending-course-url").attr("href",$(".trending-course-list-url:first").data("url")):$(".trending-course-url").attr("href",base_url+"courses");}
backgroundImagesLoader._loadForcely();$('.loader').loader();}}});});});};var trendingSlider;$.fn.trendingCourseCategories=function(){trendingSlider=$(".trend-slider").customSwiper({observer:true,pagination:{el:".swiper-pagination",clickable:true,},});if(!this.length)return;return this.each(function(){var $obj=$(this);$obj.on("click",function(){if(typeof $obj.data("value")!="number")return;$obj.parents('.select-trends').find('.dropdown-toggle').html($obj.data("text"));$.ajax({type:"get",url:base_url+"courses/ajax_get_home_courses_by_category_id/"+
$obj.data("value"),dataType:"json",async:true,cache:false,}).done(function(ajaxResponse,textStatus,jqXHR){if(jqXHR.readyState==4&&jqXHR.status==200){if(ajaxResponse.result.response==true){if(typeof ajaxResponse.result.view!="undefined"){$obj.closest(".trending-courses").find(".trending-courses-list").html(ajaxResponse.result.view);swiperWrapper=$('.trend-slider').children('.swiper-wrapper').clone(true);newSlides=$('.trend-slider').children('.swiper-slide').clone(true);trendingSlider.destroy();swiperWrapper.empty().append(newSlides);trendingSlider=$(".trend-slider").customSwiper({observer:true,pagination:{el:".swiper-pagination",clickable:true,},});$obj.val()!=0?$(".trending-course-url").attr("href",$(".trending-course-list-url:first").data("url")):$(".trending-course-url").attr("href",base_url+"courses");backgroundImagesLoader._loadForcely();$('.loader').loader();}}}});});});};})(jQuery);$(".trending-search").submit(function(e){e.preventDefault();var searchString=$(".trending-search input[name='trending_course']").val();if(searchString.length<=0){return;}
if(!searchString.replace(/\s/g,'').length){return;}
$.ajax({type:"get",url:base_url+"courses/ajax_get_home_courses_by_name",data:{search:searchString,},dataType:"json",async:true,cache:false,}).done(function(ajaxResponse,textStatus,jqXHR){if(jqXHR.readyState==4&&jqXHR.status==200){if(ajaxResponse.result.response==true){if(typeof ajaxResponse.result.view!="undefined"){displayTrendingCategories();$(".trending-courses").find(".trending-search-courses-list").html(ajaxResponse.result.view);$(".trend-search-slider").customSwiper({pagination:{el:".swiper-pagination",clickable:true,},});if(ajaxResponse.result.count===0){$(".trending-courses").find('.swiper-button-prev,.swiper-button-next').hide();}
searchResultString(searchString,ajaxResponse.result.count);backgroundImagesLoader._loadForcely();$('.loader').loader();}}}});});$(".lxp-slider").customSwiper({slidesPerView:1,spaceBetween:30,mousewheel:true,direction:"vertical",pagination:{el:".swiper-pagination",clickable:true,},navigation:{},mousewheel:{releaseOnEdges:true,},breakpoints:{},});$(".smallslid").customSwiper({slidesPerView:1,spaceBetween:12,mousewheel:true,direction:"horizontal",pagination:{el:'.lxp-small-pagination',clickable:true,},navigation:{},mousewheel:false,breakpoints:{},});$(".company-slider").customSwiper({slidesPerView:1,spaceBetween:16,autoplay:true,clickable:true,breakpoints:{},loop:true,speed:500,autoplay:{delay:5000,},});$(".accr-slids").customSwiper({slidesPerView:4,spaceBetween:16,breakpointsInverse:true,clickable:true,pagination:{el:'.swiper-pagination',dynamicBullets:true},navigation:{nextEl:'.accr-next',prevEl:'.accr-prev',},breakpoints:{1024:{slidesPerView:5,spaceBetween:16,},768:{slidesPerView:3,spaceBetween:12,},640:{slidesPerView:3,spaceBetween:12,},320:{slidesPerView:2,spaceBetween:12,}},});$(".multi_banner").customSwiper({slidesPerView:1,});$(".container-fluid").on("click",".trending-search-clear",function(){clearTrendingSearch();});function clearTrendingSearch(){$trending_courses=$(".trending-courses");$trending_courses.find("#myTab").removeClass("d-none");$trending_courses.find(".tab-content").removeClass("d-none");$trending_courses.find(".select-trends").removeClass("d-none");$trending_courses.find(".trending-search-courses-data").addClass("d-none");$trending_courses.find(".trending-search-result-text").addClass("d-none");$trending_courses.find("input[name='trending_course']").val("");$trending_courses.find('.swiper-button-prev,.swiper-button-next').show();}
function displayTrendingCategories(){$trending_courses=$(".trending-courses");$trending_courses.find("#myTab").addClass("d-none");$trending_courses.find(".tab-content").addClass("d-none");$trending_courses.find(".select-trends").addClass("d-none");$trending_courses.find(".trending-search-courses-data").removeClass("d-none");}
function searchResultString(p_strString,p_nCount){$trending_courses=$(".trending-courses");$trending_courses.find(".trending-search-result-text").find(".trending-course-search-string").html(p_strString);$trending_courses.find(".trending-search-result-text").find(".trending-course-search-count").html(p_nCount);$trending_courses.find(".trending-search-result-text").removeClass("d-none");}
$(".trending-course-category .category-list, .mobile-trending-course-category .category-list").trendingCourseCategories();var video=$("video")[0];$(".banner-play-btn").on("click",function(){if(video.paused){video.play();}else{video.pause();}});$('.left-arrows').click(function(){$('.corp-ind-image').animate({width:'toggle'},"slow");$('.corporate-image').animate({width:'toggle'},"slow");$('.left-arrows').animate({width:'toggle'},"slow");$('.right-arrows').animate({width:'toggle'},"slow");});$('.right-arrows').click(function(){$('.corp-ind-image').animate({width:'toggle'},"slow");$('.corporate-image').animate({width:'toggle'},"slow");$('.right-arrows').animate({width:'toggle'},"slow");$('.left-arrows').animate({width:'toggle'},"slow");});$(window).bind("load resize",function(e){e.preventDefault();if(window.innerWidth<=576){$('.d-flex.justify-content-center.align-items-center.corporate').hide();$('.left-arrows').click(function(){$('.corporate-sec .corporate-summary .individual').hide();$(".corporate-sec .corporate-summary .individual").removeClass("d-flex");$('.corporate-sec .corporate-summary .individual').css("display","none !important");$('.d-flex.justify-content-center.align-items-center.corporate').show();$(".corporate-sec .corporate-summary .corporate").removeClass("d-flex");});$('.right-arrows').click(function(){$('.corporate-sec .corporate-summary .corporate').hide();$(".corporate-sec .corporate-summary .corporate").addClass("d-flex");$('.corporate-sec .corporate-summary .corporate').css("display","none !important");$('.d-flex.justify-content-center.align-items-center.individual').show();$(".corporate-sec .corporate-summary .individual").addClass("d-flex");});}});;